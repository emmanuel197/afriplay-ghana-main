...
const fetchMovieSlice = createSlice({
  ...
  reducers: {
    ...
    //   // Update only when the location pathname is "/".
    //   if (window.location.pathname === "/") {
    //     state = {
    //       ...state,
    //       trending: trending?.[0]?.content || trending || [],
    //       recentlyadded: recentlyadded?.[0]?.content || recentlyadded || [],
    //       bingeworthy: bingeworthy?.[0]?.content || bingeworthy || [],
    //     };
    //   } else {
    //     // Create an object to hold the updated categories
    //     const updatedCategories = {};
    
    //     for (const [category, categoryMovies] of Object.entries(moviesByCategories)) {
    //       const categoryKey = category.toLowerCase();
    //       if (Array.isArray(state[categoryKey])) {
    //         updatedCategories[categoryKey] = categoryMovies?.[0]?.content || categoryMovies || [];
    //       }
    //     }
    //     console.log(updatedCategories)
    //     // Update state with new categories without altering other state variables
    ...
    fetchMovies_success: (state, action) => {
      ...
      if (window.location.pathname === "/") {
        ...
        //     key !== "movies" &&
        //     key !== "selectedMovie" &&
        //     protectedCategories.includes(key)
        //   ) {
        //     state[key] = [];
        //   }
        // }
